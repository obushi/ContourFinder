...
void contourfinder::ContoursExtractor::setWarpPerspectiveTransform(std::vector<ofVec2f> const & ofCaptureAreaCorners, std::vector<ofVec2f>  const & ofWindowCorners){
    cv::Mat m = cv::getPerspectiveTransform(ofxCv::toCv(ofCaptureAreaCorners), ofxCv::toCv(ofWindowCorners));
    m = m.inv();
